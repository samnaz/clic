...
<script language='javascript'>    
    ...
    function ListLessons(){
        $.ajax({
            type: "GET",
            url: "/clic/Lessons/getlessons.json",
            cache: false,
    ...
    function actionFormatter(value, row, index) {
        return [
            '<a href="/clic/Lessons/editlesson/' + row["LessonId"] + '" title="Editar">',
                '<img style="width:15px;margin:5px;" src="/clic/img/icons/icon_edit.png" alt="Editar">',
            '</a>',
            '<a href="/clic/Lessons/viewlesson/' + row["LessonId"] + '" title="Detalle">',
    ...
    function Deletelesson(id) {
        $.ajax({
            data: "",
            type: 'GET',
            url: '/clic/Lessons/deletelesson/' + id+ '.json',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function(data) {
				if(data.result)
