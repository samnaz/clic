...
<script language='javascript'>
    ...
    function ListParameters(){
        $.ajax({
            type: "GET",
            url: "/clic/Parameters/getparameters.json",
            cache: false,
    ...
    function actionFormatter(value, row, index) {
                return [
            '<a href="/clic/Parameters/editparameter/' + row["ParameterId"] + '" title="Editar">',
                '<img style="width:15px;margin:5px;" src="/clic/img/icons/icon_edit.png" alt="Editar">',
            '</a>',
            '<a href="/clic/Parameters/viewparameter/' + row["ParameterId"] + '" title="Detalle">',
